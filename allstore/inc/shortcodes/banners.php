        ...
        if (!empty($image)) :
            ...
            <div class="banner-i <?php echo esc_html($position); ?><?php if ($with_bg == 'true') echo ' with-bg'; ?>">
                ...
                <span class="banner-i-bg">
                    <img src="<?php $image_arr = wp_get_attachment_image_src( $image, 'allstore-1140x1140' );echo esc_attr($image_arr[0]);?>" alt="">
                    <?php if (!empty($subtitle) || !empty($title) || !empty($content) || (!empty($link) && !empty($link['title']))) : ?>

                    <?php if (!empty($link)) : ?>
                    <div class="banner-i-wrap" <?php if (!empty($link['target'])) echo ' target="'.esc_attr($link['target']).'"'; ?>>
                    <?php else: ?>
