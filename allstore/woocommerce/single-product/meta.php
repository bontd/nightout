...
<ul class="prod-i-props">
	...
	<?php if ( wc_product_sku_enabled() && ( $product->get_sku() || $product->is_type( 'variable' ) ) && $props_count < $props_count_max ) : ?>
		...
		<li class="sku_wrapper"><b><?php esc_html_e( 'Mã sản phẩm:', 'allstore' ); ?></b> <span class="sku"><?php echo ( $sku = $product->get_sku() ) ? $sku : esc_html__( 'N/A', 'allstore' ); ?></span></li>
	...
	if ($props_count < $props_count_max) {
		$categories = wc_get_product_category_list( $product->get_id(), ', ', '<li><b>' . _n( 'Loại sản phẩm:', 'Categories:', count( $product->get_category_ids() ), 'allstore' ) . '</b> ', '</li>' );
	...
	<?php if ( $product->has_weight() && $props_count < $props_count_max ) : ?>
		<li><b><?php esc_html_e( 'Khối lượng', 'allstore' ) ?></b> <?php echo esc_html( wc_format_weight( $product->get_weight() ) ); ?></li>
		<?php $props_count++; ?>
	<?php endif; ?>

	<?php if ( $product->has_dimensions() && $props_count < $props_count_max ) : ?>
		<li><b><?php esc_html_e( 'Kích thước', 'allstore' ) ?></b> <?php echo esc_html( wc_format_dimensions( $product->get_dimensions( false ) ) ); ?></li>
		<?php $props_count++; ?>
	<?php endif; ?>
